...
function gadget:GetInfo()
	return {
		name      = "Unit Texture Overrides",
		version   = 3,
		desc      = "Handles unit texture overrides (3do glow only, for now)",
		author    = "raaar, based on ZK commander texture swapper by Anarchid",
		date      = "2023",
		license   = "GPL V2",
		layer     = 0,
		enabled   = true
...
local IterableMap = VFS.Include("lualibs/IterableMap.lua")
...
-- TODO this will probably break if a map adds units with 3do models and textures
-- atlas positions will no longer match
local defaultTex2 = "unittextures/3do_tex2.png"
...
local function ProcessUnit(unitID, unitDefID, drawFlag, skinName)
	...
	--DEBUG
	--if unitDefID then
	--Spring.Echo("Adding unit %%48:0", UnitDefs[unitDefID].modeltype,UnitDefs[unitDefID].name, textureOverrides[unitID][1], textureOverrides[unitID][2])
	--end
...
function gadget:UnitFinished(unitID, unitDefID)
	-- skip units with non-3do models
	if UnitDefs[unitDefID].modeltype ~= "3do" then
		return
	end

	local skinName = Spring.GetUnitRulesParam(unitID, 'skin_texture')	
