...
AirJet.Default = {
  ...
  length        = 50,
  color         = {0, 0, 0.5},
  distortion    = 0.006,  -- was 0.02
  jitterWidthScale  = 3,
  jitterLengthScale = 3,
  animSpeed     = 1,

  texture1      = "bitmaps/perlin_noise.jpg", --// noise texture
  texture2      = ":c:bitmaps/jet.bmp",       --// shape
  texture3      = ":c:bitmaps/jet.bmp",       --// jitter shape
}
  ...
  jetShader = gl.CreateShader({
    ...
    fragment = [[
      ...
      void main(void)
      {
          ...
          //txCoord.s += (texture2D(noiseMap, displacement * distortion * 10.0).y - 0.5) * 20.0 * distortion;
          //txCoord.t +=  texture2D(noiseMap, displacement).x * (1.0-texCoords.t)        * 8.0 * distortion;
...
local function BeginEndDrawList(self)
  ...
  -- secondary draw list, so that the effect is visible when looking along the emit vector direction
  -- it "kind of" works..
