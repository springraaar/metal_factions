...
-- sep 2015 : split long messages over several lines instead of cutting 
-- . TODO (refactoring needed, broken code still kept)
...
function widget:GetConfigData()
	...
	-- TODO enable this again? sometimes it didn't work
	if ( False and SIZE_X > 1 and SIZE_Y > 1) then
		return {
...
function processConsoleLine(line, playerName, playerColor, playerFontStyle)
	...
	if (SEPARATE_MESSAGES == 0 or string.len(playerName) > 0) then
		...
		PLAYER_MSG_HISTORY[NUM_PLAYER_MESSAGES + 1] = {playerColor, playerFontStyle, getTimeStr()..line}
		...
		SYSTEM_MSG_HISTORY[NUM_SYSTEM_MESSAGES + 1] = getTimeStr()..line
